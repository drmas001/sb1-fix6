...
const PatientDischarge: React.FC = () => {
  ...
  const handlePatientSelect = async (patient: Patient | Consultation) => {
    ...
    try {
      const table = patient.mrn.startsWith('C-') ? 'consultations' : 'patients';
                    ...
                    {previousVisits.map((visit) => (
                      <li key={visit.mrn + visit.admission_date} className="py-2">
                        <p className="text-sm text-gray-600">
                          {visit.mrn.startsWith('C-') ? 'Consultation' : 'Admission'}: {new Date(visit.admission_date).toLocaleDateString()} {visit.admission_time}
                        </p>
                        {visit.updated_at && (
                          <p className="text-sm text-gray-600">
